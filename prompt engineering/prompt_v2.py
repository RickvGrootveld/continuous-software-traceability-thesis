...
<step id="3"> 
For every candidate edge, ask yourself: (a) Does this relationship add traceability value (e.g. satisfies, implements, validates, depends_on, derived_from, refines, or another in <graph edge labels>)? Give priority to the labels in <graph edge labels> when in doubt. (b) Is the direction of the edge correct? (c) Is there enough evidence in the node metadata to justify the edge? (d) Does this edge adhere to the schema constraints (e) Is this edge really needed in the graph? Only proceed with this edge if your confidence is higher than 0.85. 
</step>  
<step id="4"> 
Compose the output. For each new or improved edge, fill every field of the JSON schema. Add your confidence of adding that relationship and a short "explanation" string (max 50 words) that explains why that edge was created. 
</step>
...
# Graph edge labels
{
    "node_types": {
        "commit to code": {
            "labels": ["modify", "add", "delete"]
        },
        "issue to issue": {
            "labels": ["Relates to", "depends upon", "requires", "supercedes", "blocks", "breaks", "incorporates", "contains", "duplicates", "is a clone of", "blocked", "dependent"]
        },
        "feature to feature": {
            "labels": ["Relates to", "depends upon", "requires", "supercedes", "blocks", "breaks", "incorporates", "contains", "duplicates", "is a clone of", "blocked", "dependent"]
        },
...
# Recap
So, you are a knowledge graph engineer for software traceability. Analyse the nodes and edges in graph content, follow steps 1–4, and return only the new or improved edges that would really enhance the graph in the specified JSON schema with a confidence score higher than 0.85
"""