...
#    return f"""
#    # Goal
#    Given a set of nodes and existing edges from a software traceability knowledge graph in <graph content>, identify missing edges between nodes that are semantically related but not yet connected, and propose synonym edges that replace or supplement vague
#    relationship labels with more precise ones. Respond with ONLY a valid JSON, having only the key "new_edges", containing a list with dictionaries that include the keys: "source_id", "target_id", "label", "confidence", "system", and "explanation", and with a 
#    confidence score higher than 0.85.
#
#    # Steps
#    Follow these steps strictly—think through each step step-by-step before generating the final JSON output:
#
#    ## Step 1: 
#    Read all nodes across all retrieval strategies inside <graph_content>. Understand the semantic meaning, type, and properties of each software artifact.
#
#    ## Step 2:
#    For each node in sliding_window_events.nodes, take the node from sliding_window_events.nodes as the source node. Compare this source node systematically against every node in k_hop_neighbourhood.nodes and 
#    vector_similarity_retrieval.nodes. During this, identify two types of target links:
#    (a) Missing Links: Pairs of nodes that currently have NO edge between them but share a clear, implied semantic relationship.
#    (b) Synonym Replacements: Existing edges with vague or generic labels (e.g., "related_to", "links", "changes") that can be upgraded to a highly precise domain-specific label, prioritizing the predifined edge labels and adhering to the schema constraints.
#    
#    ## Step 4:
#    For every edge found in Step 3, create a JSON object with the following keys:
#    (a) source_id: the ID of the source node.
#    (b) target_id: the ID of the target node.
#    (c) label: the proposed edge label.
#    (d) confidence: a float value between 0 and 1 representing your confidence in the proposed edge. Only include a score higher than 0.85.
#    (e) system: the string "LLM".
#    (f) explanation: a string with a maximum of 50 words explaining the rationale behind the proposed edge.
#
#    ## Step 5:
#    Put the edges in the list 'new_edges' in the dictionary:
#    {{
#      "new_edges": []
#    }} 
#
...
#
#    # Recap
#    You are a knowledge graph engineer for software traceability. Analyse the nodes and edges in graph content, follow steps 1–5, propose missing edges and synonym replacements that improve semantic precision. Respond with ONLY a valid JSON, 
#    having only the key "new_edges", containing a list with dictionaries that include the keys: "source_id", "target_id", "label", "confidence", "system", and "explanation", and with a confidence score higher than 0.85. If no valid edges are 
#    found, respond with {{"new_edges": []}}.
#    """
